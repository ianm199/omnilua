...
fn number_to_str_buf(val: &LuaValue, version: lua_types::LuaVersion) -> Vec<u8> {
    ...
    match val {
        ...
        LuaValue::Float(f) => {
            ...
            // 5.3+ append ".0" to an integer-valued float so it reads as a
            // float (the int/float distinction). 5.1/5.2 are float-only and have
            // no such distinction, so they print `5`, not `5.0`.
